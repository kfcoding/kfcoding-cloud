...
```
<dependency>
    <groupId>com.cuiyun</groupId>
    <artifactId>kfcoding-cache</artifactId>
...
表达式举例 | 说明 | 举例
-------------  |------------- | -----
@Cache(key="user:{1}")<br>public User getUserByAccount(String account) | {1}表示获取第一个参数值<br>{2}表示获取第二个参数值<br>……依此类推 | 若：account = kfcoding，则：key = user:kfcoding
